...
export const createWorkerWithDataUrl = (f: () => void, options?: WorkerOptions) =>
  new Worker(functionToDataUrl(f), options)

export const createSharedWorkerWithDataUrl = (f: () => void, options?: SharedWorkerOptions) =>
  new SharedWorker(functionToDataUrl(f), options)

export const createWorkerWithBlob = (f: () => void, options?: WorkerOptions) => {
...
export const createSharedWorkerWithBlob = (f: () => void, options?: SharedWorkerOptions) => {
  const url = functionToObjectUrl(f)
...
export const functionToObjectUrl = (f: () => void) =>
  URL.createObjectURL(new Blob([`(${f.toString()})()`], { type: 'text/javascript' }))
...
export const createSharedWorker = (f: () => void, options?: SharedWorkerOptions) => {
  try {
